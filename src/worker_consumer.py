...
class TemporalWorkerConsumerProvider(framework.Object):
    ...
    def _on_worker_consumer_changed(
        self, event: RelationChangedEvent | RelationJoinedEvent | ConfigChangedEvent
    ):
...
class TemporalWorkerConsumerRequirer(framework.Object):
    ...
    def _on_worker_consumer_relation_changed(self, event: RelationChangedEvent):
        ...
        try:
            ...
            self.charm.unit.status = WaitingStatus("Waiting for temporal-worker-consumer relation")
            event.defer()
            return
